            ...
            if (token) {
                ...
                try {
                    if (decoded.role === "admin") {
                        ...
                        const res = await axios.get(TOTALREGISTEREVENT, {
                            headers: { Authorization: `Bearer ${token}` },
                            withCredentials: true,
                        });
                        setEvents(res.data.evntDetails || []);
                    }
                } catch (err) {
        ...
        <div className="min-h-screen bg-gradient-to-br from-background via-purple-light/30 to-background p-4 sm:p-6 mt-20">
            <div className="max-w-4xl mx-auto space-y-8">
                        ...
                        <div className="relative">
                            <Avatar className="h-20 w-20 sm:h-24 sm:w-24 ring-4 ring-purple-primary/20">
                                <AvatarImage
                                    src={selectedAvatar}
                                    alt={user.name}
                                    onError={() => setSelectedAvatar(avatar)}
                                />
                ...
                <Card className="p-4 sm:p-6 shadow-lg border-purple-light/50">
                    ...
                    {events.length === 0 && user.role === "user" ? (
                        <p>User has not registered for any event yet.</p>
                    ) : (
